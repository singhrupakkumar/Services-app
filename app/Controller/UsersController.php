                    ...
                    if (empty($this->data['User']['username'])) {
                        ...
                        if ($fu['User']['email']) {
                            ...
                            if ($fu['User']['active'] == 1) {
                                ...
                                $ms = "<p>Welcome To Service app</p><p>Click the Link below to reset your password.</p><br /> " . $url;
    ...
    public function admin_edit($id = null) {
	   ...
	   $admin_edit = $this->User->find('first', array('conditions' => array('User.id' => $id)));	
            if ($this->User->save($this->request->data)) {
                ...
                $this->Session->setFlash('The user has been saved');
                if($admin_edit['User']['role']=='provider'){
                  return $this->redirect(array('action' => 'index'));    
                }else{ 
...
public function api_verify($id = null) {
 ...
 public function api_resetpwd($token = null) {
    ...
    public function api_view() {
        ...
        if ($getuser) {
            $getuser['User']['name'] =  $getuser['User']['firstname'].' '.$getuser['User']['lastname'];
            $response['status'] = true;
	...
	public function api_registration() { 
		...
		if ($this->request->is('post')) {
			...
			$emailCount = $this->User->find('first', array('conditions' => array('User.email' => $email)));
			if(!empty($emailCount)){
				    $response['status'] = false;
				    $response['msg'] = 'Email already exists!';
                                     $response['data'] =  $emailCount ;
