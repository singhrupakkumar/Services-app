...
/////////////////////All Api///////////////////////////////////////
  ...
  ////////////////////////////////////////////////////////////
    ...
    public function admin_providerservice($id = null) {
        Configure::write("debug", 0);
         $this->loadModel('Providerservice');
        $Providerservice = $this->Providerservice->find('all',array('recursive'=>1,'conditions'=>array('Providerservice.provider_id'=>$id)));
        
        $ids = array();
        foreach($Providerservice as $k){
            
         $ids[] =  $k['Providerservice']['service_id'];  
        }
        $this->set('services', $this->Service->find('all',array('conditions'=>array('Service.id'=> $ids)))); 
        $this->set('provider_id',$id); 
    }
	...
	public function admin_availabledates($id = null) {
		...
		$this->loadModel('User');
		 ...
		 $provider = $this->User->find('first',array('conditions'=>array(
                        'AND'=>array(
                            'User.id'=>$id,
                            'User.role'=>'provider'    
                        )
...
////////////////////////////////////////////////////////////
    ...
    public function admin_add($provider_id = null) {
     ...
     public function admin_provideradd($provider_id = null) {  
	   $this->loadModel('Providerservice');
        if ($this->request->is('post')) {

            foreach ($this->request->data['service'] as $data){
                $this->request->data['Providerservice']['service_id'] = $data; 
                 $this->request->data['Providerservice']['provider_id'] = $provider_id;
                    $this->Providerservice->create();
                 $save = $this->Providerservice->save($this->request->data);  
            }
		...
		$this->set('servicelist', $this->Service->find('list',array('conditions'=>array('Service.status'=>1))));
...
////////////////////////////////////////////////////////////
	...
	$this->set('Category', $this->Category->find('list'));
        $this->set('SubCategory', $this->SubCategory->find('list'));
    }
