...
<section class="content">
    <div class="row">
		...
		<div class="box-header">
                    <h3 class="box-title">All Approved Bills</h3>
                </div>
 ...
 <div class='searchByDate'>
	...
	$d = explode(' ', $user['Bill']['approved_date']);
	$d1 = explode('-', $d[0]);
	$monthNum  = $d1[1];
	
	$dateObj   = DateTime::createFromFormat('!m', $monthNum);
	$monthName = $dateObj->format('F');
	$date_approve = $monthName.' '.$d1[0];
	?>
    <tr data-date = "<?php echo $date_approve; ?>">
		...
		<td><?php 
         $approved_date = explode(' ', $user['Bill']['approved_date']);
         $date1 = str_replace('-', '/', $approved_date[0]);
         echo date('m-d-Y',strtotime($date1)); ?></td>
        <td><?php 
         $due_date = explode(' ', $user['Bill']['due_date']);
         $date2 = str_replace('-', '/', $due_date[0]);
         echo date('m-d-Y',strtotime($date2)); ?></td>
        <td><?php 
         $created = explode(' ', $user['Bill']['created']);
         $date3 = str_replace('-', '/', $created[0]);
         echo date('m-d-Y',strtotime($date3)); ?></td>
       
	   <td class="actions">
            <?php echo $this->Html->link('View', array('action' => 'view', $user['Bill']['id']), array('class' => 'btn btn-primary btn-xs view1')); ?>				
            <?php echo $this->Form->postLink('Delete Bill', array('action' => 'approvedelete', $user['Bill']['id']), array('class' => 'btn btn-danger  btn-xs'), __('Are you sure you want to delete # %s?', $user['Bill']['title'])); ?>
        </td>
    </tr>
    <?php endforeach; ?>
    </tbody>
